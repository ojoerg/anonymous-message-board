...
async function find10ThreadsByBumpedOn(boardId) {
  var result = await Thread.find({})
    .where("board_id", boardId)
    .sort({ bumped_on: "desc" })
    .slice("replies", -3)
    .limit(10)
    .select(
      "-reported -delete_password -__v -replies.reported -replies.delete_password"
    );
  return result;
}

async function findSingleThreadById(threadId) {
  var result = await Thread.findOne({ _id: threadId }).select(
    "-reported -delete_password -__v -replies.reported -replies.delete_password"
  );
  return result;
}
...
async function findReply(threadId, replyId) {
  var result = await Thread.findOne({
    _id: threadId,
...
async function addReply(threadId, replies, date) {
  var result = Thread.findOneAndUpdate(
    { _id: threadId },
    { replies: replies, bumped_on: date }
  );
  return result;
}
...
async function postThread(
  ...
  try {
    ...
    var threadCreate = await createThread(
      boardFound._id,
      text,
      deletePassword,
      date,
